...
class AvatarRenderer:
    ...
    def _draw(cls, state):
        p = PALETTE
        img = Image.new("RGBA", (cls.BASE_SIZE, cls.BASE_SIZE), p["transparent"])
        ...
        if state == "sleep":
            ...
            d.rectangle((32,15,34,17), fill=p["cyan"]); d.rectangle((34,12,37,14), fill=p["cyan"]); d.rectangle((36,8,39,10), fill=p["cyan"])
        elif state == "error":
            d.line((9,21,17,27), fill=p["brown"], width=3); d.line((17,21,9,27), fill=p["brown"], width=3)
            d.line((23,21,31,27), fill=p["brown"], width=3); d.line((31,21,23,27), fill=p["brown"], width=3)
            d.line((15,33,19,30,23,33,27,30), fill=p["brown"], width=2)
            d.rectangle((35,4,38,12), fill=p["red"]); d.rectangle((35,14,38,17), fill=p["red"])
        else:
            ...
            if state == "listening":
                ...
                d.arc((0,15,8,32),275,85,fill=p["cyan"],width=3); d.arc((32,15,40,32),95,265,fill=p["cyan"],width=3)
            elif state == "thinking":
                d.ellipse((18,31,22,33), fill=p["brown"]); d.arc((32,4,39,12),200,500,fill=p["orange_light"],width=3); d.rectangle((35,14,38,17), fill=p["orange_light"])
            elif state == "answer":
                d.rectangle((15,29,25,35), fill=p["brown"]); d.rectangle((17,31,23,33), fill=p["blush"])
                d.polygon([(33,24),(39,21),(37,27)], fill=p["orange_light"]); d.polygon([(34,30),(39,31),(35,34)], fill=p["orange_light"])
            else:
                d.arc((15,29,25,35),5,175,fill=p["brown"],width=2)
        if state not in {"sleep","error"}:
            d.rectangle((8,29,11,30), fill=p["blush"]); d.rectangle((29,29,32,30), fill=p["blush"])

        # Keep the pixel-art edges crisp while increasing the actual display tile to 64x64.
        return img.resize((cls.SIZE, cls.SIZE), Image.Resampling.NEAREST)

    @staticmethod
    def _eye(draw,x,y,wide=False,look_right=False):
        p=PALETTE; rx,ry=((5,6) if wide else (4,5)); draw.ellipse((x-rx,y-ry,x+rx,y+ry),fill=p["white"],outline=p["outline"],width=1)
        px=x+(2 if look_right else 0); draw.ellipse((px-3,y-3,px+3,y+4),fill=p["amber_dark"]); draw.rectangle((px-2,y,px+2,y+3),fill=p["amber"]); draw.rectangle((px-2,y-3,px,y-1),fill=p["white"])

def install_avatar_hook():
    if getattr(builtins,"_whisplay_avatar_hook_installed",False): return
    original_build_class=builtins.__build_class__; builtins._whisplay_avatar_hook_installed=True
    def build_class(func,name,*bases,**kwargs):
        cls=original_build_class(func,name,*bases,**kwargs)
        if name!="RenderThread": return cls
        original_render_header=getattr(cls,"render_header",None)
        if original_render_header is None:
            builtins.__build_class__=original_build_class; return cls
        def render_header_with_avatar(self,image,draw,status,emoji,battery_level,battery_color):
            ...
            if avatar is None: return result
            main=sys.modules.get("__main__"); status_font_size=getattr(main,"status_font_size",20); terminal_text=getattr(main,"current_terminal_text","")
            x=(self.whisplay.LCD_WIDTH-AvatarRenderer.SIZE)//2
            if terminal_text: x=self.whisplay.CornerHeight
            ...
            return result
        cls.render_header=render_header_with_avatar; builtins.__build_class__=original_build_class; return cls
